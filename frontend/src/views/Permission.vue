<template>
  <div>
    ...
    <div>
      <el-button size="mini" type="primary" @click="showAddDialog">新增</el-button>
      <el-table :data="tableData" style="width: 100%;margin-bottom: 20px;" row-key="id" border default-expand-all
        :tree-props="{ children: 'children', hasChildren: 'hasChildren' }">
        <el-table-column prop="comment" label="名称" sortable width="180"></el-table-column>
        ...
        <el-table-column prop="name" label="组件名" sortable width="180"></el-table-column>
    ...
    <el-dialog title="添加权限菜单" :visible.sync="addDialogVisible" width="40%" center>
      <el-form :model="newForm" ref="newForm" label-width="80px" size="small" :rules="rules">
        <el-form-item label="权限名称" prop="comment">
          <el-input v-model="newForm.comment" autocomplete="off"></el-input>
        </el-form-item>
        ...
        <el-form-item v-if="newForm.menuType === '普通权限菜单'" label="组件名" prop="name">
          <el-input v-model="newForm.name" autocomplete="off" placeholder="Permission"></el-input>
        </el-form-item>

        <el-form-item v-if="newForm.menuType === '普通权限菜单'" label="路径" prop="path">
          <el-input v-model="newForm.path" autocomplete="off" placeholder="/permission"></el-input>
        </el-form-item>
        <el-form-item v-model="newForm.parentId" label="父节点" prop="parentId">
          <div class="block">
            <el-cascader v-model="selectedValue" :options="cascaderOptions"
              :props="{ expandTrigger: 'click', checkStrictly: true }" @change="handleChange" size="large"
              clearable></el-cascader>
          </div>
        </el-form-item>
        <el-form-item label="图标" prop="icon">
          <el-select v-model="newForm.icon" clearable placeholder="请选择图标"
            v-if="iconDictionary && iconDictionary.length">
            <el-option v-for="item in iconDictionary" :key="item.label" :label="item.label" :value="item.value">
              <i :class="item.value" style="margin-right: 30px;"></i>{{ item.label }}
            </el-option>
          </el-select>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="编辑权限菜单" :visible.sync="editDialogVisible" width="40%" center>
      <el-form :model="editForm" ref="editForm" label-width="80px" size="small" :rules="rules">
        <el-form-item label="权限名称" prop="comment">
          <el-input v-model="editForm.comment" autocomplete="off"></el-input>
        </el-form-item>

        <el-form-item v-if="editForm.menuType === '普通权限菜单'" label="组件名" prop="name">
          <el-input v-model="editForm.name" autocomplete="off"></el-input>
        </el-form-item>

        <el-form-item v-if="editForm.menuType === '普通权限菜单'" label="路径" prop="path">
          <el-input v-model="editForm.path" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item v-model="editForm.parentId" label="父节点" prop="parentId">
          <div class="block">
            <el-cascader v-model="editForm.parentId" :options="cascaderOptions"
              :props="{ expandTrigger: 'click', checkStrictly: true }" size="large" clearable></el-cascader>
          </div>
        </el-form-item>
        <el-form-item label="图标" prop="icon">
          <el-select v-model="editForm.icon" clearable placeholder="请选择图标"
            v-if="iconDictionary && iconDictionary.length">
            <el-option v-for="item in iconDictionary" :key="item.label" :label="item.label" :value="item.value">
              <i :class="item.value" style="margin-right: 15px;"></i>{{ item.label }}
            </el-option>
            <!-- 选中后将图标与文字一起回显 -->
            <template slot="prefix" v-if="editForm.icon">
              <i :class="editForm.icon" style="margin-left: 10px;"></i>
            </template>
          </el-select>
...
export default {
  ...
  methods: {
    ...
    load() {
      this.request.get("/permission").then(response => {
    ...
    handleDelete(row) {
      this.$confirm('确定删除该项吗?', '删除确认', {
        ...
        this.request.delete("/permission/delete/" + row.id).then(response => {
    ...
    handleEdit() {
      ...
      this.request.post("/permission/saveOrUpdate", form)
        .then(response => {
          if (response && response.code === '200') {
            ...
            if (response.msg && response.msg.includes("已存在，请选择其他名称")) {
    ...
    handleAdd() {
      if (this.selectedValue.length > 0) {
        this.newForm.parentId = this.selectedValue[this.selectedValue.length - 1]; // 取最后一个值作为父节点ID
      }
      this.$refs.newForm.validate((valid) => {
        ...
        if (valid) {
          this.request.post("/permission/saveOrUpdate", this.newForm)
            .then(response => {
              if (response && response.code === '200') {
                reloadActiveRotes(this);
